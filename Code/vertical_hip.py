...
while cap.isOpened():
    ...
    if result.pose_landmarks:
        ...
        # Draw a vertical line from the initial hip point (static reference)
        cv2.line(frame, (int(initial_hip_point[0]), 0), (int(initial_hip_point[0]), h), (255, 0, 0), 1)  # Blue line

        # Draw a vertical dotted line from the current hip point (moving with hip)
        for i in range(0, h, 20):
            cv2.line(frame, (int(current_hip_point[0]), i), (int(current_hip_point[0]), i+10), (0, 0, 255), 1)  # Red dotted line
