...
# List to store the coordinates and frame numbers
data = []
...
while cap.isOpened():
    ...
    if result.pose_landmarks:
        ...
        draw_dotted_circle(frame, (int(initial_centroid[0]), int(initial_centroid[1])), 10, (255, 0, 0), 1, 5)  # Dotted blue circle

        # Draw a circle at the current centroid in all frames (moving with head)
        cv2.circle(frame, (int(current_centroid[0]), int(current_centroid[1])), 10, (0, 0, 255), 1)  # Red circle with outer line only
        ...
        # Store the coordinates and frame number
        data.append({
            "frame_no": frame_no,
            "initial_centroid": {
                "x": initial_centroid[0],
                "y": initial_centroid[1],
                "z": initial_centroid[2]
            },
            "current_centroid": {
                "x": current_centroid[0],
                "y": current_centroid[1],
                "z": current_centroid[2]
            }
        })
...
print("Coordinates and frame numbers have been saved to centroid_data.json")
